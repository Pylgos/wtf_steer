 ...
 public:
  ...
  void update(std::array<float, 4> present_drive_ang_vels, std::array<Angle, 4> present_steer_angles,
    ...
    for(size_t i = 0; i < units_.size(); i++) {
      auto error = units_[i].get_angle_error(present_drive_ang_vels[i], present_steer_angles[i] - offset_[i]);
      errors *= std::cos(error.rad());
    }
    for(size_t i = 0; i < units_.size(); i++) {
      units_[i].set_tgt_vel(units_[i].get_tgt_vel() * errors);
